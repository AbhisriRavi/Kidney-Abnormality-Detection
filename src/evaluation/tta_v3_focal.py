"""
Test-time augmentation for v3 focal+balanced checkpoints.

For each of the 15 fold-model checkpoints (3 seeds × 5 folds):
  1. Load model
  2. For each test image, run 5 TTA versions and average softmax
  3. Compute test metrics (accuracy, per-class F1, macro-F1, macro-AUC)
  4. Save per-fold metrics.json

Then aggregate across 15 folds and compare to non-TTA baseline.
"""
import os
import json
...
OUT = SCRATCH / "kidney-results/v3_tta"
...
SEEDS = [42, 123, 456]
RUN_TAGS = {
    42:  "v3_rn50_focal_run1_seed42",
    123: "v3_rn50_focal_run1_seed123",
    456: "v3_rn50_focal_run1_seed456",
}
MANIFESTS = {
    42:  V3_DIR / "manifest_with_folds.csv",
    123: V3_DIR / "manifest_with_folds_seed123.csv",
    456: V3_DIR / "manifest_with_folds_seed456.csv",
}


# TTA transforms: identity, hflip, rot+5, rot-5, center-zoom
...
class KidneyDatasetTTA(Dataset):
    """Returns a single test image and its label. TTA transforms applied in eval loop."""
...
def tta_predict(model, test_df, batch_size=32):
    """Predict on test_df with TTA, returning averaged softmax probs and labels."""
    n = len(test_df)
    ...
    for tta_i, tf in enumerate(TTA_TFS):
        # Custom collate: apply the current TTA transform to each PIL image
        def collate(batch):
...
for seed in SEEDS:
    ...
    for fold_idx in range(5):
        ...
        for c in CLASSES:
            print(f"    {c:<8} F1={fold_result['per_class'][c]['f1']:.4f}")
