"""
Per-dataset training script for Chapter 5.

Trains one architecture on one dataset with patient-grouped 5-fold CV,
mirroring the v3_baseline protocol. Each dataset uses its own native
class set (Mendeley 4-class, KiTS 3-class, Abdalla 2-class).

Usage:
  python train_per_dataset.py --dataset mendeley --arch resnet50 --seed 42
"""
import os
import csv
import json
...
if args.tag is None:
    args.tag = f"pd_{args.dataset}_{args.arch}_seed{args.seed}"

SCRATCH = Path(os.environ["SCRATCH"])
MANIFEST = SCRATCH / "kidney-data/processed/per_dataset" / args.dataset / f"manifest_seed{args.seed}.csv"
...
# Determine class set based on dataset
DATASET_CLASSES = {
...
print(f"Dataset: {args.dataset} ({N_CLASSES}-class: {CLASSES})")
...
for fold_idx in folds_to_run:
    ...
    print(f"  {args.arch}: {n_params/1e6:.1f}M parameters")
    ...
    for i, c in enumerate(CLASSES):
        try:
            if N_CLASSES == 2:
                aucs[c] = float(roc_auc_score(t_t == i, t_pr[:, i]))
            else:
    ...
    ax.set_title(f"{args.dataset}/{args.arch} fold {fold_idx} | acc={acc:.3f} F1={macro_f1:.3f}")
    plt.tight_layout()
    plt.savefig(fold_out / "confusion_matrix.png", dpi=110, bbox_inches="tight")
    plt.close()

    print(f"  Fold {fold_idx} test: acc={acc:.4f}, macro-F1={macro_f1:.4f}, AUC={macro_auc}")
