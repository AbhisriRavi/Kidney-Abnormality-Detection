"""
KiTS linear probe experiment.

Freezes the entire ResNet50 backbone. Only the final classification head trains.
This tests whether KiTS 38% is a data ceiling or a training artefact.
"""
import os
import csv
import json
...
parser.add_argument("--lr", type=float, default=1e-3)  # higher than usual since only fc trains
...
if args.tag is None:
    args.tag = f"pdlp_kits_resnet50_seed{args.seed}"
...
print(f"KiTS linear probe: lr={args.lr} wd={args.weight_decay} epochs={args.epochs}")
...
def build_frozen_model(n_classes):
    """ResNet50 with all backbone params frozen. Only fc layer trains."""
    m = models.resnet50(weights=models.ResNet50_Weights.IMAGENET1K_V2)
    # Freeze every parameter first
    for p in m.parameters():
        p.requires_grad = False
    # Replace the fc head with a new trainable linear layer
    m.fc = nn.Linear(m.fc.in_features, n_classes)
    # fc is trainable by default; verify
...
for fold_idx in folds_to_run:
    ...
    model = build_frozen_model(N_CLASSES).to(DEVICE)
    ...
    # Only optimize trainable params (fc layer)
    ...
    for epoch in range(args.epochs):
        model.train()
        # Batchnorm layers stay in eval mode when their weights are frozen
        # This is important for linear probe -- prevents running stats drift
    ...
    ax.set_title(f"KiTS linear probe fold {fold_idx} | acc={acc:.3f} F1={macro_f1:.3f}")
    plt.tight_layout()
