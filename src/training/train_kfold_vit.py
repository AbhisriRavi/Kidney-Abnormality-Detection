"""
K-fold training with Vision Transformer (ViT-Base-Patch16-224).
Identical to train_kfold_v2.py except the model is timm's ViT.
"""
import os
import csv
import json
...
parser.add_argument("--batch-size", type=int, default=32)   # smaller for ViT
parser.add_argument("--lr", type=float, default=5e-5)        # lower for ViT
...
for fold in range(N_FOLDS):
    ...
    # ViT-Base, pretrained on ImageNet, 4-class head
    model = timm.create_model("vit_base_patch16_224",
                              pretrained=True, num_classes=len(CLASSES))
    model = model.to(DEVICE)
    n_params = sum(p.numel() for p in model.parameters()) / 1e6
    print(f"  Model: ViT-Base/16-224, {n_params:.1f}M params")
    ...
    aucs = {c: float(roc_auc_score((t_t == i).astype(int), t_pr[:, i]))
            if 0 < (t_t == i).sum() < len(t_t) else None
